...
class DataReader(BaseReader):
    ...
    def _read_and_decode(self, filename_queue):
        ...
        y = tf.reshape(tf.decode_raw(features["y"].values, tf.float32),
                       [Config.num_response])
        seq_len = features["seq_len"]
        context = tf.constant([0.0]*Config.context_size)

        return x, y, seq_len, context
    ...
    def _read_input(self, file_name):
        ...
        with tf.name_scope("input"):
            ...
            x, y, seq_len, context = self._read_and_decode(filename_queue)

            # Collect examples into batch_size batches
            # We run this in three threads to avoid being a bottleneck
            x_batch, y_batch, seq_len_batch, context_batch = tf.train.batch(
                [x, y, seq_len, context], batch_size=Config.batch_size,
                num_threads=3, capacity=100 + 3 * Config.batch_size,
                dynamic_pad=True, allow_smaller_final_batch=True)

        return x_batch, y_batch, seq_len_batch, context_batch
    ...
    def get_batch(self):
        ...
        """
        x_batch, y_batch, seq_len_batch, context_batch = self._read_input("input")
