...
class ResourcePool:
	...
	def getAvailableNodeList(self, **kwargs):
		...
		for dev, ch in zip(self.resource_list, map(lambda res: res.attributes, self.resource_list.values())):
		...
		for key in reservDescriptor:
			if key == "network" or key == "storage" or key == "router":
				...
				for item in reservDescriptor[key]:
					...
					available_attributes = self.resource_list[mid].devices[self.__id(mid)]["Available"]
...
def start_server(port):
	...
	try:
		...
		cinfo = filter(lambda line: "USERNAME" in line, open(os.path.join(Environment.CONF,\
	...
	pid = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE).communicate()[0].strip()
	
	if not (pid in [None, ""]):

		pid = int(pid)
		subprocess.Popen("kill -9 %d" % int(pid), shell=True, stdout=subprocess.PIPE).wait()
