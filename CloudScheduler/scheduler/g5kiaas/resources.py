    ...
    def __get_jobs(self):
        ...
        if self.jobs == []:
            ...
            for reservation in self.reserve:
                ...
                job_id = filter(lambda line: line.startswith("OAR_JOB_ID"), output.split())[0]
                job_id = int(job_id.split('=')[1].strip())
                
                self.jobs.append({"Site" : site, "Cluster" : cluster, "jobID" : job_id})
        ...
        for site  in self.MACHINES:       
            for c in self.MACHINES[site].values():
                for ip in map(lambda mac:mac["ip"], c["Machines"]):
                    ...
                    subprocess.Popen("ssh %s chmod +x /root/mond" % ip, shell=True, stdout=subprocess.PIPE).wait()

                    cmd = "nohup python /root/mond &"

                    subprocess.Popen("ssh %s %s" % (ip, cmd), shell=True, stdout=subprocess.PIPE).wait()
...
class ResourceSet:
    ...
    def __init__(self, site, cluster, resources):
        ...
        for m in resources["Machines"]:
            ...
            self.devices[m["ip"]] = {"Available" : resources["Characteristics"].copy(), "Hostname" : m["hostname"],
